...
public class DBWrapper extends DB
{
    ...
    /**
	 * Read a record from the database using singleton transaction. Each field/value pair from the result will be stored in a HashMap.
	 *
	 * @param table The name of the table
	 * @param key The record key of the record to read.
	 * @param fields The list of fields to read, or null for all of them
	...
	public int singletonRead(String table, String key, Set<String> fields, HashMap<String,ByteIterator> result) {
		long st=System.nanoTime();
		int res=_db.singletonRead(table,key,fields,result);
		long en=System.nanoTime();
		_measurements.measure("SINGLETON READ",(int)((en-st)/1000));
		_measurements.reportReturnCode("SINGLETON READ",res);
		return res;

	}
	
	/**
	 * Update a record in the database using singleton transaction. Any field/value pairs in the specified values 
	 * HashMap will be written into the record with the specified record key, overwriting any existing values with the same field name.
	 *
	 * @param table The name of the table
	 * @param key The record key of the record to write.
	 * @param values A HashMap of field/value pairs to update in the record
